...
// //   useEffect(() => {
// //     getLogo();
// //     fetchJobApplicationData(currentPage, itemsPerPage).then((result) => {
// //       dispatch(setJobApplicationData(result.data));
...
// //                 {totalRecords} Recently Added Jobs
// //               </h6>
// //             </div>
// //           </div>
// //           <div className="row">
...
//     toast("Login To Continue");
//   };
// console.log(jobApplicationData,"jad");
//   const toggleFabJobs = async (id) => {
//     try {
//       await axios({
//         url: "https://api.novajobs.us/api/jobseeker/job-favorites",
...
function Jobsection() {
  ...
  const fetchJobApplicationData = async (page = 1, perPage = 5) => {
    try {
      ...
      dispatch(setJobApplicationData(sortedData)); // Update the Redux store
      setTotalRecords(response.data.total_records); // Update the total records
      setSkeleton(false);
    } catch (error) {
      console.error("Error fetching job data:", error);
      dispatch(setJobApplicationData([])); // Reset the data on error
      setSkeleton(false);
    }
  };
  ...
  const handlePageChange = (pageNumber) => {
    if (pageNumber > 0 && pageNumber <= Math.ceil(totalRecords / itemsPerPage)) {
      setCurrentPage(pageNumber);
          ...
          <div className="row">
            <div className="col-lg-9">
              ...
              <div className="pagination d-flex flex-wrap justify-content-center">
                ...
                {/* {Array.from({ length: Math.ceil(totalRecords / itemsPerPage) }).map(
                  (_, index) => (
                    <button
                      key={index}
                      onClick={() => handlePageChange(index + 1)}
                ...
                {Array.from({ length: totalRecords }, (_, index) => {
                  if (
                    index < 5 ||
                    (index === totalRecords - 1 && totalRecords > 5)
                  ) {
                    return (
                      <button
                        key={index}
                  ...
                  } else if (index === 5 && totalRecords > 5) {
                    return (
                      <span key="ellipsis" className="me-2 mb-2">
                        ...
                      </span>
